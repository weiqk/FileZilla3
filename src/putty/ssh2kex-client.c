...
void ssh2kex_coroutine(struct ssh2_transport_state *s, bool *aborted)
{
    ...
    if (s->kex_alg->main_type == KEXTYPE_DH) {
        ...
        if (dh_is_gex(s->kex_alg)) {
            ...
            fzprintf(sftpKexAlgorithm, "%d-bit Diffie-Hellman with server-supplied group", dh_modulus_bit_size(s->dh_ctx));
            fzprintf(sftpKexHash, ssh_hash_alg(s->exhash)->text_name);
            ...
            fzprintf(sftpKexAlgorithm, "%d-bit Diffie-Hellman with standard group", dh_modulus_bit_size(s->dh_ctx), s->kex_alg->groupname);
            fzprintf(sftpKexHash, ssh_hash_alg(s->exhash)->text_name);
            ppl_logevent("Doing Diffie-Hellman key exchange using %d-bit "
        ...
        fzprintf(sftpKexHash, ssh_hash_alg(s->exhash)->text_name);
        fzprintf(sftpKexCurve, ssh_ecdhkex_curve_textname(s->kex_alg));
